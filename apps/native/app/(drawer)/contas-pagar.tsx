...
const statusVariant = (status: string) => {
  switch (status) {
    ...
    case "Aprovado": return "default" as const;
...
function ContasPagarContent() {
  ...
  const [editingId, setEditingId] = useState<Id<"contasPagar"> | null>(null);
  const [payingId, setPayingId] = useState<Id<"contasPagar"> | null>(null);
  const [formData, setFormData] = useState<FormData>(emptyForm);
  const [payForm, setPayForm] = useState({ dataPagamento: "", formaPagamento: "", contaBancariaId: "" });
  ...
  const fornecedorOptions = useMemo(
    () => (fornecedores ?? []).filter((f: any) => f.isActive !== false).map((f: any) => ({ label: f.name, value: f._id })),
    [fornecedores],
  );
  ...
  const filtered = useMemo(() => {
    ...
    return contas.filter((c) => {
      ...
      if (search) {
        ...
        const matchFornecedor = c.fornecedor?.name?.toLowerCase().includes(term);
        if (!matchDesc && !matchFornecedor) return false;
      }
      return true;
    });
  ...
  const handleCreate = async () => {
    try {
      await createConta({
        ...
        fornecedorId: formData.fornecedorId ? (formData.fornecedorId as Id<"suppliers">) : undefined,
  ...
  const handlePay = async () => {
    ...
    try {
      await pagarConta({
        id: payingId,
        dataPagamento: payForm.dataPagamento ? new Date(payForm.dataPagamento).getTime() : undefined,
        formaPagamento: payForm.formaPagamento ? (payForm.formaPagamento as any) : undefined,
        contaBancariaId: payForm.contaBancariaId ? (payForm.contaBancariaId as Id<"contasBancarias">) : undefined,
      });
      setPayingId(null);
      setPayForm({ dataPagamento: "", formaPagamento: "", contaBancariaId: "" });
    } catch (e: any) {
  ...
  const handleAprovar = (id: Id<"contasPagar">) => {
    Alert.alert("Aprovar", "Deseja aprovar esta conta?", [
  ...
  const handleCancelar = (id: Id<"contasPagar">) => {
  ...
  return (
    <View className="gap-4">
      <View className="flex-row items-center justify-between">
        <View>
          <Text className="text-2xl font-bold text-foreground">Contas a Pagar</Text>
          <Text className="text-muted-foreground text-sm">Gerencie suas despesas</Text>
        </View>
      ...
      <View className="flex-row gap-3">
        <View className="flex-1">
          <View className="flex-row items-center border border-input rounded-md bg-card px-3 h-10">
            ...
            <Input
              className="flex-1 border-0 h-10"
              placeholder="Buscar..."
              value={search}
      ...
      <Dialog open={!!payingId} onOpenChange={(open) => !open && setPayingId(null)}>
        ...
        <View className="gap-3 mt-3">
          <View className="gap-1">
            <Label>Data do Pagamento (AAAA-MM-DD)</Label>
            <Input
              value={payForm.dataPagamento}
              onChangeText={(t) => setPayForm((f) => ({ ...f, dataPagamento: t }))}
          ...
          <View className="gap-1">
            ...
            <Select
              ...
              onValueChange={(v) => setPayForm((f) => ({ ...f, formaPagamento: v }))}
          ...
          <View className="gap-1">
            ...
            <Select
              ...
              onValueChange={(v) => setPayForm((f) => ({ ...f, contaBancariaId: v }))}
        ...
        <DialogFooter>
          <Button variant="outline" onPress={() => setPayingId(null)}>
            <ButtonText variant="outline">Cancelar</ButtonText>
          </Button>
          <Button onPress={handlePay}><ButtonText>Confirmar Pagamento</ButtonText></Button>
        </DialogFooter>
      </Dialog>
                ...
                <View key={conta._id} className="rounded-lg border border-border p-3">
                  <View className="flex-row items-start justify-between">
                    <View className="flex-1">
                      ...
                      <Text className="text-muted-foreground text-xs mt-0.5">
                        {conta.fornecedor?.name ?? "Sem fornecedor"}
                      </Text>
                    </View>
                  ...
                  <View className="flex-row items-center gap-2 mt-2 pt-2 border-t border-border">
                    {(conta.status === "Pendente" || conta.status === "Vencido") && (
                      <Pressable
                        onPress={() => handleAprovar(conta._id)}
                        className="flex-row items-center gap-1 px-2 py-1 rounded bg-blue-50"
                      >
                        <CheckCircle2 size={14} color="#3b82f6" />
                        <Text className="text-xs text-blue-600">Aprovar</Text>
                      </Pressable>
                    )}
                    {conta.status !== "Pago" && conta.status !== "Cancelado" && (
                      <Pressable
                        onPress={() => {
                          setPayingId(conta._id);
                          setPayForm({ dataPagamento: "", formaPagamento: conta.formaPagamento ?? "", contaBancariaId: conta.contaBancariaId ?? "" });
                        }}
                        className="flex-row items-center gap-1 px-2 py-1 rounded bg-emerald-50"
                      >
                        <CreditCard size={14} color="#10b981" />
                        <Text className="text-xs text-emerald-600">Pagar</Text>
                      </Pressable>
                    )}
                    {conta.status !== "Pago" && conta.status !== "Cancelado" && (
                      <Pressable
...
function FormFields({
  ...
  fornecedorOptions: { label: string; value: string }[];
  ...
  return (
    <View className="gap-3 mt-3">
      <View className="gap-1">
        ...
        <Input value={formData.descricao} onChangeText={set("descricao")} placeholder="Ex: Aluguel escritório" />
      </View>
      <View className="flex-row gap-3">
        ...
        <View className="flex-1 gap-1">
          ...
          <Input value={formData.dataVencimento} onChangeText={set("dataVencimento")} placeholder="2025-01-15" />
        </View>
      </View>
      <View className="gap-1">
        <Label>Data Competência (AAAA-MM-DD)</Label>
        <Input value={formData.dataCompetencia} onChangeText={set("dataCompetencia")} placeholder="2025-01-01" />
      </View>
      <View className="gap-1">
      ...
      <View className="gap-1">
        ...
        <Select value={formData.fornecedorId} onValueChange={set("fornecedorId")} options={fornecedorOptions} placeholder="Selecione..." />
      </View>
      <View className="gap-1">
        <Label>Conta Bancária</Label>
