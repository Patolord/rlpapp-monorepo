...
export const NORMA_LABELS = {
    1: "ABNT",
    2: "ABNT v.2008",
    3: "ABNT (c/ reforço TR)",
};
export const EXTERNAL_INSULATION_LABELS = {
...
export const INTERNAL_INSULATION_LABELS = {
    none: "S = sem isol.",
    bidim: "B = Bidim",
    flexiliner: "F = Flexiliner",
};
export const FLANGE_LABELS = {
    none: "S = sem flange",
...
export const BOM_SECTION_LABELS = {
...
export function defaultDuctPrices() {
    return { ...DEFAULT_PRICES };
}
export function defaultDuctEstimateInput() {
    return {
        norma: 1,
        laborRatePerKg: 18,
...
export function isFilledDuctLine(line) {
    return (Boolean(line.tag?.trim()) ||
        line.largerSideCm > 0 ||
        line.smallerSideCm > 0 ||
        line.lengthM > 0);
}
export function mergeDuctPrices(prices) {
...
function sheetAreaM2(line) {
    return (((line.largerSideCm / 100 + line.smallerSideCm / 100) * 2) * line.lengthM);
}
function perimeterM(line) {
    return ((line.largerSideCm + line.smallerSideCm) / 100) * 2;
}
export function selectGauge(largerSideCm, norma) {
    if (!(largerSideCm > 0))
        return null;
    const limits = GAUGE_LIMITS[norma];
    if (largerSideCm <= limits.g26)
        return "26";
    if (largerSideCm <= limits.g24)
        return "24";
    if (largerSideCm <= limits.g22)
        return "22";
    if (largerSideCm <= limits.g20)
        return "20";
    return "18";
}
function zeroSheetArea() {
    return { "26": 0, "24": 0, "22": 0, "20": 0, "18": 0 };
}
function computeRawLine(line, input) {
    ...
    const alum = line.externalInsulation === "manta"
        ? excelRound(line.lengthM + line.lengthM * peri * 2 * 1.1)
        : 0;
    const angleSheet = ((input.insulationThicknessMm / 1000 + 0.02) * line.lengthM) * 4 * 1.3;
    const raw = {
        sheetArea,
        perimeter: peri,
        ...
        pw1: line.flange === "powermatic" && line.largerSideCm <= 200 ? flangeLen : 0,
        pw2: line.flange === "powermatic" && line.largerSideCm > 200 ? flangeLen : 0,
        ...
        pwClamps: line.flange === "powermatic" && line.largerSideCm > 30 ? joints * 4 : 0,
...
function sumRaw(lines) {
    ...
    for (const line of lines) {
        for (const gauge of Object.keys(SHEET_DENSITY)) {
...
function withAllowance(value, pct) {
    return value * (pct / 100 + 1);
}
export function computeDuctEstimate(input) {
    ...
    const rawSheetKg = sheetKgRaw["26"] +
        sheetKgRaw["24"] +
        sheetKgRaw["22"] +
        sheetKgRaw["20"] +
        sheetKgRaw["18"] +
    ...
    const qty = {
        ...
        sheet26Angle: excelRound(summed.angleIsopor * SHEET_DENSITY["26"] +
        ...
        glue: 0,
        coldAsphalt: 0,
        nylonTape: summed.nylonManta + summed.nylonIsopor + summed.nylonPlaca,
        ...
        rivets: 0,
        pwTape: 0,
        angleFlange: excelRound((summed.angleFlangeM + summed.angleOver220) * 2.66 * 1.3),
    };
    qty.glue = excelRound(((qty.isopor + qty.manta + qty.placa + qty.bidim) * 0.45) / 3.6);
    qty.coldAsphalt = excelRound(qty.isopor / 80);
    const paintArea = excelRound(withAllowance(summed.paint, insPct) +
    ...
    const bom = BOM_ITEM_DEFS.map((def) => {
